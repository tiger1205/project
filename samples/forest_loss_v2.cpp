...
for (int i = 0;i < years; i ++ ) { 
 	...
 	*(pt.access(i,j)) = ({int k; 
k = ((i*season_length)+j); ( ((k>=cols)) ? ((0.0-25)) : *( data.access(row, k))  )
; })
  ;} } 
...
int y; 
float score1; 
score1 = 0.0;for (x = 0 ;x <= yr; x++ )	{ 
for (y = (yr+1) ;y <= (years-1); y++ )	{ 
score1 = (score1+*( comparisonMatrix.access(x, y)) ); } 

 } 

score1 = ((score1*2)/(yr*(years-yr)));float score2; 
score2 = 0.0;for (x = 0 ;x <= yr; x++ )	{ 
for (y = 0 ;y <= yr; y++ )	{ 
score2 = (score2+*( comparisonMatrix.access(x, y)) ); } 

 } 

score2 = (score2/(((yr-1)*yr)/2));float score3; 
score3 = 0.0;for (x = (yr+1) ;x <= (years-1); x++ )	{ 
for (y = (yr+1) ;y <= (years-1); y++ )	{ 
score3 = (score3+*( comparisonMatrix.access(x, y)) ); } 

 } 

score3 = (score3/(((years-yr)*((years-yr)-1))/2)); ((score1-score2)-score3); })
  ;} } 
float maximum; 
maximum = (0.0-25);int k; 
k = 0;for (k = 0 ;k <= (years-1); k++ )	{ 
